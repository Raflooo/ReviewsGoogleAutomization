...
def _open_reviews_tab(page: Page):
    ...
    # Intento 2: por texto visible "Opiniones" o "Reseñas" (sin depender del rol)
    try:
        text_tab = page.get_by_text(re.compile(r"^(Opiniones|Reseñas|Reviews)$", re.IGNORECASE))
        if text_tab.count() > 0:
            text_tab.first.click(timeout=8000)
            page.wait_for_timeout(2000)
            return
    except Exception as e:
        print(f"  [debug] Intento 2 (texto Opiniones) falló: {e}")

    # Intento 3: el rating con estrellas (ej: "5,0 estrellas, 12 reseñas")
    ...
    try:
        ...
        print(f"  [debug] Intento 3 (rating/estrellas) falló: {e}")

    # Intento 4: por atributo data-tab-index (así lo marca Google internamente)
    try:
        ...
        print(f"  [debug] Intento 4 (data-tab-index) falló: {e}")

    # Intento 5: el selector viejo por aria-label (por si Google vuelve a usarlo)
    try:
        ...
        print(f"  [debug] Intento 5 (aria-label) falló: {e}")
